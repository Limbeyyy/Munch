...
the event they are arriving for is open or it is not. The room belongs to
the event - one room holding the whole running order - so the quarter of
an hour is counted from the event, and the gaps between one talk and the
...
class GuestDoorWindowTests(TestCase):
    ...
    def day(self, *slots, status=Event.Status.SCHEDULED):
        """A event whose sessions run at the given offsets from now."""
        first = slots[0][0]
        event = make_event(self.host,
        ...
        for offset, length in slots:
            Session.objects.create(
                event=event, title=f'Session at {offset}',
    ...
    def test_earlier_than_that_the_room_is_not_ready(self):
        # Event at 9; it is 8:30.
        event = self.day((30, 60))
    ...
    def test_it_says_when_to_come_back(self):
        event = self.day((45, 60))

        body = self.knock(event).json()

        opens = timezone.datetime.fromisoformat(body['opens_at'])
        self.assertEqual(
            (event.scheduled_start - opens).total_seconds() / 60, 15
        )

    def test_arriving_late_to_a_live_session_is_fine(self):
        event = self.day((-20, 60), status=Event.Status.ACTIVE)
        session = event.sessions.first()
        session.status = Session.Status.LIVE
        session.started_at = session.starts_at
        session.save()

        self.assertEqual(self.knock(event).status_code, 201)

    def test_in_the_gap_between_two_sessions_the_room_is_still_open(self):
        # The event is under way, but nothing is on stage: the first
        # session finished twenty minutes ago and the next is an hour off.
        # The room is the event's, and it goes on between talks - the
        # host is setting up for the next speaker, not closing up.
        event = self.day((-80, 60), (60, 60), status=Event.Status.ACTIVE)
        done = event.sessions.order_by('starts_at').first()
    ...
    def test_a_quarter_of_an_hour_before_the_second_session_the_door_is_open(self):
        event = self.day((-80, 60), (10, 60), status=Event.Status.ACTIVE)
        done = event.sessions.order_by('starts_at').first()
        done.status = Session.Status.DONE
        done.save()
    ...
    def test_a_session_running_long_is_still_something_to_come_in_for(self):
        ...
        event = self.day((-120, 30), status=Event.Status.ACTIVE)
        session = event.sessions.first()
        session.status = Session.Status.LIVE
        session.started_at = session.starts_at
        session.save()
    ...
    def test_a_meeting_that_has_finished_says_so_without_a_time(self):
        event = self.day((-200, 60), status=Event.Status.ENDED)
    ...
    def test_a_meeting_with_nothing_scheduled_falls_back_to_its_own_window(self):
        # The running order has not been filled in. It is still a event
        # that is happening, so the old rule answers.
        event = make_event(self.host, start=timezone.now() + timezone.timedelta(minutes=5)
        )

        self.assertEqual(self.knock(event).status_code, 201)

    def test_and_says_when_it_opens_rather_than_talking_about_sessions(self):
        event = make_event(self.host, start=timezone.now() + timezone.timedelta(hours=3)
        )
