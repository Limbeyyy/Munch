...
class IntervalTests(TestCase):
    def setUp(self):
        ...
        self.event = make_event(self.host, start=self.start, minutes=600)
        self.first = make_session(self.event, self.start, 60, 'Haldi')
    ...
    def test_without_a_preference_it_is_the_old_fifteen(self):
        self.assertEqual(gap_minutes_for(self.event), GAP_MINUTES)

    def test_the_hosts_own_interval_is_what_counts(self):
        self.set_gap(45)

        self.assertEqual(gap_minutes_for(self.event), 45)

    def test_the_next_session_may_start_after_that_interval(self):
        self.set_gap(45)

        soonest = earliest_start(self.event)
    ...
    def test_and_the_refusal_quotes_the_hosts_own_number(self):
        ...
        with self.assertRaises(ScheduleConflict) as refused:
            check_slot(self.event, self.start + timezone.timedelta(minutes=90), 30)
    ...
    def test_moving_a_session_reflows_by_the_hosts_interval(self):
        self.set_gap(30)
        second = make_session(
            self.event, self.start + timezone.timedelta(minutes=90), 60, 'Sagun'
        )
    ...
    def test_zero_means_back_to_back(self):
        ...
        check_slot(self.event, self.start + timezone.timedelta(minutes=60), 30)
...
class ReminderSettingTests(TestCase):
    ...
    def setUp(self):
        ...
        self.session = make_session(self.event, self.start, 60, 'Haldi')
    ...
    def test_without_a_preference_it_is_an_hour_and_a_quarter_of_one(self):
        ...
        self.assertEqual(
            (self.event.scheduled_start - rows['event'].due_at).total_seconds() / 60,
            60,
        )
        self.assertEqual(
    ...
    def test_the_hosts_own_warning_is_what_is_used(self):
        self.set_leads(event_reminder_minutes=180, session_reminder_minutes=45)
    ...
    def test_the_page_quotes_the_warning_it_actually_gave(self):
        ...
        self.assertEqual(leads, {'event': 120, 'session': 30})
    ...
    def test_the_settings_endpoint_carries_them(self):
        ...
        client.post(
            ...
            {'event_reminder_minutes': 90, 'reminders_enabled': False},
            content_type='application/json',
        )

        body = client.get(f'{API}/users/scheduling/').json()
        self.assertEqual(body['event_reminder_minutes'], 90)
    ...
    def test_more_than_a_day_of_warning_is_refused(self):
        response = self.client_for(self.host).post(
            f'{API}/users/scheduling/', {'event_reminder_minutes': 2000},
            content_type='application/json',
        )

        self.assertEqual(response.status_code, 400)
        self.assertEqual(response.json()['field'], 'event_reminder_minutes')
